...
// 主題分享功能
const shareThemeBtn = document.querySelector('.share-theme');
...
// 生成主題分享連結
function generateThemeShareLink() {
    const customTheme = localStorage.getItem('customTheme');
    if (!customTheme) return '';
    
    const currentUrl = window.location.href.split('?')[0];
    const themeData = encodeURIComponent(customTheme);
    return `${currentUrl}?theme=${themeData}`;
}
...
function loadSharedTheme() {
    ...
    if (themeData) {
        try {
            ...
            console.error('無法載入分享的主題：', error);
        }
    }
}

// 點擊分享按鈕
shareThemeBtn.addEventListener('click', () => {
    const shareLink = generateThemeShareLink();
    if (shareLink) {
